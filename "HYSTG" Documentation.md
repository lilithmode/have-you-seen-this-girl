## 1/19/25
- finally planned out stuff and wrote it down lol
	- cast finally decided
		- lauren as nun 
		- celadon as angel
	- general plot is as follows:
		- the nun is essentially a big parasocial person, who really wants to connect with this higher deity of some sort.
			- she's also, a bit, well, kind of wild. she can say some of the worst things imaginable. when inebriated.
		- the angel is someone that she sees as holy, pious, pure of heart.
				- representation of denial, a coping mechanism
			- in reality, this is someone who has not thought about "the nun" in ages. the cope has settled in SO HARD that they start relating to them as an angel / religious deity of some sort. maybe it's real, maybe it isn't. fuck them.
### 3 ACT STRUCTURE:
##### ACT I
- talking with angel
	- hide away (accept offer) (1)
	- face fear (reject offer) (2)
##### ACT II
- talk with angel (1)
- confront actual person (2)
##### ACT III
- denial (1) (live but it sucks lol)
-  acceptance (2) (live lol)
## 1.20.26
- happy first day of school lol
	- something that came to mind is i have no idea how i'm going to be organizing these stems -- *do i need to "make" the composition again in quad? or do I export all the stems and run it through supacollider?*
- three compositions that have "routes" will probably be the most reasonable for me to work on like a NUT
- last composition would have the ending route they chose's ending - very simple. nothing secret.
	- ACT I 
		- begrudgingly (intro)
			- the angel descends
		- babble (1)
		- violence against the hands that fed me?! (2)
	- ACT II
		- blahblahblahblah (1)
		- why does it hurt when i talk to you? (2)
	- ACT III
		- fetal once more (1)
		- like beams of light, extended towards the sky (2)
			- the angel ascends (1)
			- the angel leaves (2)
- Fuck. *FUCK.* i gotta do alla this. but i think the scope feels manageable
-  how many tracks is that?  **10?** fuckkkkkk.
	- ending survey would FUCK HARDDDDDD - google forms?
## 1.21.26
- talked with celadon last night
	- availability depends, but we should be able to finish script NEXT WEEK
- lauren availability NEXT WEEK
- enna availability:
	- Week of Feb 1
		- Prep session on Feb 6
		- Real session Feb 7th
			- admittedly, will work on OBSESSIONS more than anything, but i'm looking to get some weird samples and other tidbits
		- i'm hungry
- yeah let's get fucking cooking chat first day of capstone tomorrow
## 1.22.26 - 1.24.26
- first class ever!! everyone is so nice!!
- I have got to get my schedule done ASAP. I also have to see if renpy is compatible with supercollider FR by the end of this week
	- everything literally lies and depends on this. do the damn work TONITE. 
- [x] start script ✅ 2026-01-24
- [x] make schedule ✅ 2026-01-26
-  [ ] start composition ideas (PLEASE GOD)
- [x] figure out visuals another way via supercollider (but interactive) ✅ 2026-01-31
	- parasocial link between vtubing and true crime
		- nah it's about human connection and shit lol
		- “You don’t have to live up to myths or expectations. Being seen is not a performance. You are allowed to exist quietly and imperfectly, and that is enough.”

## 1.26.26
- **finished scheduling.**
	- that was a bit harder than i expected
	- but i hope that a lot of it will be reasonable with my workload and extra curriculars. i will be eating, shitting, and sleeping supercollider after the ableton stuff.
	- let's go foodman!

## 1.27.26
- https://docs.supercollider.online/Classes/Scheduler.html
	- read this again lol
- https://doc.sccode.org/Guides/Multichannel-Expansion.html
	- how tf do i make this in quad lmao
- i'm going to start writing the script and just letting it happen
	- wrote the climax, the big phone call. lack of angel. now i just have to write around it. 
- have the beginning have the nun talk to the phone booth / angel talking about what she did for her day.
	- make it boring but see that she has a general love for life, but there's something always bothering her around that time.
- sai - is the angel always looking for the nun to be in a negative frame of mind until you do your duties?

## 1.28.26
- ok. scope creep already got to me. i think i need to keep it focused.
	- writing script (again)
- i need to write context docs by tn
	- or i just need to finish the script 
		- the angel has the nun act as a missionary
	- ok i got some decent progress done ! let's finish this shit.

## 1.29.26
- big writing day
- cherubim, seraphims
	- via alex - take a look @ that concept :3
- ~~expand what the credits do~~
	- no more credits, actually.
	- exposition is important !!
	- nick: are the people down bad when they enter the phone booth
	- zen: assumes that there is a certain level of ambiguity

## 1.30.26
- writing...

## 1.31.26
- finally finished first pass of the script
	- recorded the nun YAAYYYY
	- need to record:
		- the child
		- the worker
		- the prostitute
		- the angel
		- the ex
- i need to get this done NEOWWWWW

## 2.1.26
- happy february
	- finish the nun and the child
		- planning to record the worker tn
	- started composition work !
		- relinquish instruments for each character
	- kalil
		- will help out with visuals
		- slide show of "evidence" and characters

## 2.4.26
- finished the fucking script.
- recording celadon today
	- setting up layout for the phone calls
	- finishing up instrumental demos
- first wip rendered today!

## 2.5.26
- first presentation! not too bad
	- deeptha - work on mix to make voices more legible
	- rachel - DO NOT CALL IT AN INSTALLATION
		- this is more of a multichannel radio drama
	- https://www.scribd.com/document/721693661/Multichannel-Audio-Dramas 
		- this is so funny lmfao
